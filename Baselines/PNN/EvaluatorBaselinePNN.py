...
class NetPNN(SaveableNNModule):
    def __init__(self, name, input_dim, input_ch, output_dim, output_activation=None, past_models=None):
        ...
        # CNN Layer 1
        self.conv1 = nn.Conv2d(input_ch, 64, kernel_size=5)
        ...
        if self.num_prev_models > 0:
            ...
            self.V_2 = nn.Conv2d(64*self.num_prev_models, 64, kernel_size=1)
            self.U_2 = nn.Conv2d(64, 32, kernel_size=5, bias=False)
        ...
        # FC Layer 1
        self.fc1_input_dim = self.pool2_output_dim**2*32
        if self.num_prev_models > 0:
            self.scalar_3 = nn.Parameter(torch.from_numpy(np.ones(1, dtype=np.float32)))
            self.V_3 = nn.Conv2d(32*self.num_prev_models, 32, kernel_size=1)
            self.U_3 = nn.Linear(self.fc1_input_dim, 300, bias=False)

        self.fc1 = nn.Linear(self.fc1_input_dim, 300)
        ...
        if self.output_dim is not None:
            if self.num_prev_models > 0:
                ...
                self.V_4 = nn.Linear(300 * self.num_prev_models, 300)
                self.U_4 = nn.Linear(300, output_dim, bias=False)
...
class NetPNN_RNN(SaveableNNModule):
    def __init__(self, name, input_dim, input_ch, recogniser_output_dim, recogniser_output_activation,
        ...
        if self.num_prev_models > 0:
            ...
            self.V_6 = nn.Linear(self.hidden_dim*self.num_prev_models, self.hidden_dim)
    ...
    def forward(self, x_list, return_activations=False):
        ...
        prev_activations["rnn"].append(activations)
