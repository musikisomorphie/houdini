...
from HOUDINI.Synthesizer.AST import mkRealTensorSort, mkBoolTensorSort, mkFuncSort, mkListSort
...
def testProgGenerator():
    ...
    # Tensor[real][1,1]
    sort = PPTensorSort(param_sort=PPReal(), shape=[
                        PPDimConst(value=1), PPDimConst(value=1)])
...
def testAvoidDimConstUnk():
    term = PPFuncApp(fn=PPVar(name='lib.zeros'), args=[
                     PPTermNT(name='Z', sort=PPDimConst(value=1))])
    ntId = 1
