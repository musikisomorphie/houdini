...
from HOUDINI.Synthesizer.Utils import ASTUtils, RuleUtils, MiscUtils, ReprUtils, SubstUtils
...
class SymbolicSynthesizer(BaseSynthesizer[AST.PPTerm, Action]):
    ...
    def _giveUniqueNamesToUnks(self, st: AST.PPTerm):
        def rename(nt: AST.PPTermUnk):
            return AST.PPTermUnk("nn_fun_%s_%d" % (self.nnprefix, self._ntNameGen()), nt.sort)
